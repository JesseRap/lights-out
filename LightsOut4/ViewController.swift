...
//  LightsOut4
//
//  Created by Jesse Rappaport on 8/28/16.
//  Copyright © 2016 Jesse Rappaport. All rights reserved.
//
...
class ViewController: UIViewController {
    ...
    let images: [UIImage] = [UIImage(named: "lights-on")!, UIImage(named: "lights-out")!]
    ...
    @IBAction func pressButton(sender: UIButton) {
        let tag = sender.tag
        let (x,y) = (Int(floor( Double(tag) / 3.0 )), tag % 3)
        toggleCell(sender)
        if x > 0 {
